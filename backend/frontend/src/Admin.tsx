...
type Post = { id:number; title:string; summary:string; body:string; category:Category; event_date:string|null; published:boolean; promoted:boolean; created_at:string }
...
const blankPost = { title:'', summary:'', body:'', category:'news_events' as Category, event_date:'', published:true, promoted:false }
...
export default function Admin() {
  ...
  async function savePost(event:FormEvent) {
    event.preventDefault(); setNotice('')
    try {
      await api(editingId ? `/api/posts/${editingId}` : '/api/posts',{method:editingId?'PUT':'POST',body:JSON.stringify({...editor,event_date:editor.event_date||null})})
      setEditor(blankPost); setEditingId(null); setNotice(editingId?'Post updated.':'Post published.'); await loadData()
    } catch(error) { setNotice(error instanceof Error ? error.message : 'Unable to save') }
  }

  function edit(post:Post) {
    setEditor({...post,event_date:post.event_date||''}); setEditingId(post.id); setSection('posts'); window.scrollTo({top:0,behavior:'smooth'})
  }
